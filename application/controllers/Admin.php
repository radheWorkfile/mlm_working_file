...
class Admin extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['donation']  = $this->db_model->sum('amount', 'donation_record');
    ...
    function donar_reward_list()
    {
        if ($this->input->post()) {
            $con = array('id' => $this->input->post('userid'));
            $this->db->select('id,name,email,phone')->from('member')->where('level_completed>=', 3)->where($con);
            $member = $this->db->get()->result_array();
        } else {
            $this->db->select('id,name,email,phone')->from('member')->where('level_completed>=', 3);
            $member = $this->db->get()->result_array();
        }

        foreach ($member as $i => $in) {
            $paid = $this->db_model->sum('amount', 'earning', array('userid' => $in['id'], 'type' => 'Donar Reward'));
            $member[$i]['paid'] = ($paid) ? $paid : 0;;
        }
        $data['member'] = $member;
        $data['title']      = 'Donar Reward';
        $data['breadcrumb'] = 'Donar Reward';
        $data['layout']     = 'donar/list.php';
        $this->load->view('admin/base', $data);
    }
    ...
    function pay()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->session->set_flashdata("common_flash", "<div class='alert alert-danger'>" . validation_errors() . "</div>");
            redirect('admin/donar_reward_list');
        } else {

            $val = $this->input->post();
            $this->Earning->pay_earning(trim($val['int_id']), '', 'Donar Reward', trim($val['amount']));
            $this->session->set_flashdata("common_flash", "<div class='alert alert-success'>Donar reward credited successfully.</div>");
            redirect('admin/donar_reward_list');
        }
    }
}
