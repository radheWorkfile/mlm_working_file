...
class Member extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['left_downline']=$downline_report['PersonCount']+1;
        $this->ASarray = array();
        $downline_report=$this->active_subscriber($details->B);
        $data['right_downline']=$downline_report['PersonCount']+1;
       
        $data['level']=$this->db_model->select('income_name', 'level_wise_income',array('id'=>$details->level_completed));
        $data['topup_with']=$this->db_model->select('auto_topup_with', 'level_wise_income',array('id'=>$details->level_completed));
