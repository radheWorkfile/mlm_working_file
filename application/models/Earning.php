 ...
 */
    public function level_wise_income()
    {
        ...
        foreach ($reward as $res) {
            ...
            $this->db->select('userid')->from('level')->where($con);
            $data = $this->db->get()->result();
            ...
            foreach ($data as $result) {
                ...
                if ($count <= 0) {
                    ...
                    $this->db->insert('earning', $array);
                     ...
                     ## auto topup member start ##
                    $dat=array(
                        // 'topup' =>$res->auto_topup_with,
                        'level_completed' =>$res->id
                    );
                    $con=array(
                        'id' => $result->userid,
                    );
                    $this->db->where($con)->update('member',$dat);
                    echo $this->db->last_query()."<br>";

                    // auto topup record start
                    $dat=array(
                        'user_id'=>$result->userid, 
                        'topup_amount'=>$res->auto_topup_with,
                        'topup_by'=>'Auto Upgrade',                        
                        'date'=>date('Y-m-d'),                        
                    );
                    $this->db->insert('topup_record', $dat);
                    echo $this->db->last_query()."<br>";
                    // auto topup record end

                    ##auto topup member end ##

                    ## Donation Deposite start ##

                    $dat=array(
                        'userid'=>$result->userid, 
                        'amount'=>$res->donation_amount,
                        'level'=>$res->id,
                        
                    );
                    $this->db->insert('donation_record', $dat);
                    echo $this->db->last_query()."<br>";
                    ## Donation Deposite end ##
